...
export default class BuildMaterial extends Service {
  ...
  private uploadService: CnpmService;
  ...
  async prepare(id:string | number, materialRes: I_Response, buildParam: I_MaterialBuildParam): Promise<I_Response>{
    ...
    // 创建task
    const taskRes: I_Response = await this.service.task.create({
    ...
    await this.start(materialRes, taskRes.data.id, buildParam);
  ...
  async start(materialInfo: any,taskId: string | number, buildParam: I_MaterialBuildParam) {
    ...
    if(!materialInfo.npm_name){
      ...
      this.materialInfo.npm_name = `@opentiny/lowcode-${process.env.RUN_MODE || 'dev'}-material-${materialName}-${materialInfo.id}`;
    }

    this.taskId = taskId;
    this.buildService = new MaterialBuildService(this.app);
    this.uploadService = new CnpmService(this.ctx);
